...
def main():
    ...
    app_path = os.path.join(os.getcwd(), 'frontend')
    print(f"Ищем frontend по пути: {app_path}")

    included_directories = ['lib'] #  'ios', 'android'
    included_extensions = {'.dart', '.swift', '.kt', '.java', '.gradle', '.xml', '.plist', '.pbxproj', '.xcconfig', '.properties', '.json'}

    excluded_directories = ['__pycache__', 'node_modules', '.git', 'venv', 'build', '.dart_tool', '.idea', 'DerivedData', 'GeneratedPluginRegistrant']
    excluded_extensions = {'.pyc', '.log', '.tmp', '.lock', '.jks', '.keystore', '.DS_Store', '.iml'}
