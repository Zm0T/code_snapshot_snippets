...
def main():
    ...
    # Используем абсолютный путь до frontend
    frontend_path = os.path.join(os.getcwd(), 'frontend')
    print(f"Ищем frontend по пути: {frontend_path}")

      included_directories = ['Views', 'Models', 'ViewModels']
    included_extensions = {'.swift'}

    excluded_directories = ['__pycache__', 'node_modules', '.git', 'venv']
    excluded_extensions = {'.pyc', '.log', '.tmp', '.plist', }
